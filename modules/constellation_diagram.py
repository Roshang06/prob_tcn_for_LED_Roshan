...
class QPSK_Constellation(ConstellationDiagram):
    ...
    def _generate_bit_map(self, complex_symbols):
        ...
        bits = build_bits(bits_per_symbol)
    ...
    def bits_to_symbols(self, bits: str) -> np.array:
        ...
        if len(bits) % 2 != 0:
            # Padd by a zero
            bits = bits + "0"
        grouped_bits = [bits[x : x + self.bits_per_symbol] for x in range(0, len(bits), self.bits_per_symbol)]
...
class RingShapedConstellation(ConstellationDiagram):
    def __init__(self, filename: Union[str, np.ndarray]):
        ...
        self.bits_per_symbol = int(np.log2(len(symbols)))
        self.modulation_order = len(symbols)
    ...
    def bits_to_symbols(self, bits: str) -> np.ndarray:
        if len(bits) % self.bits_per_symbol != 0:
            bits = bits.ljust(len(bits) + (self.bits_per_symbol - len(bits) % self.bits_per_symbol), '0')
        grouped = [bits[i:i+self.bits_per_symbol] for i in range(0, len(bits), self.bits_per_symbol)]
